%% Experiment 24 - Multivariate algorithm
% Maps out where new tests of convergence fail
...
% Initialization
itermax = P+1; N = 201;
testu2x= linspace(16.635,16.639,N);
testu2y= linspace(16.635,16.639,N);
nonlinsolves  =10;
...
for j = 1:N
    for k = 1:N
        ...
        while iter < itermax
            ...
            u2b = BC2 * u2; u2b = u2b(2:end-1); % G(y1,y2)
            testvalue1 = u2b-fx;
            ...
            u2b = BC2 * u2; u2b = u2b(2:end-1); % G(N(y1,y2))
            
            testvalue2 = u2b-u2bold;
            
            if sign(testvalue0'*testvalue1) ~= sign(testvalue0'*testvalue2)
                u2b = (u2bold + fx)./2;
                u2bold = u2b;
                ...
                testvalue3 = u2b-u2bold;
                
                if sign(testvalue0'*testvalue3) ~= sign(testvalue0'*testvalue1)
                    plot(fx(1),fx(2),'k.')
                else
                    plot(fx(1),fx(2),'r.')
                end
