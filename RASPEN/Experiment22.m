%% Experiment 22 - Webbing
% Repeating Exp18 plotting paths of start points
...
% Initialization
L = 40; N = 100;
testu2 = L*[sin(2*pi.*(1:N)/N); cos(2*pi.*(1:N)/N)];
Gp= zeros(P,N,2);
Gpnorm=zeros(P,N);
nonlinsolves  =10;

for k = 1:N
    fx     = [testu2(1,k), testu2(2,k), testu2(2,k), testu2(1,k)];
    u2b    = fx;
    u2bold = u2b;
    iter   = 1;
    ...
    while iter < P+1
        ...
        Gpnorm(iter,k)=norm(u2b)./norm(u2bold);
        
        u2b = u2bold + 0.1*(u2b-u2bold);
        
        Gp(iter,k,1)=u2bold(1);
        Gp(iter,k,2)=u2bold(2);
